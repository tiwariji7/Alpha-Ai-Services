...
export const PartnershipIntro: React.FC = () => {
  const pillars = [
    ...
    },
    {
      ...
      icon: <Wrench className="w-5 h-5 text-[#2A3FA8]" />,
    ...
    },
    {
      ...
      icon: <TrendingUp className="w-5 h-5 text-[#5B4FE0]" />,
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-10 border border-[#EDEAFB] shadow-soft text-left space-y-8">
        <div className="max-w-3xl space-y-3">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-[#151235] tracking-tight">
            More Than a Service. <span className="text-[#3B4FD9]">A Long-Term Technical Partner.</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          {pillars.map((pillar, idx) => (
            <motion.div
              ...
              className="bg-[#F6F5FC] p-5 sm:p-6 rounded-2xl border border-[#EDEAFB] hover:border-[#3B4FD9]/40 transition-all duration-300 flex flex-col justify-between space-y-4 group"
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-extrabold text-[#5B5876] tracking-widest uppercase">
                  {pillar.step}
                </span>
              </div>
