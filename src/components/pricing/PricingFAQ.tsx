          ...
          return (
            <div
              ...
              className={`bg-white rounded-2xl border transition-all duration-200 overflow-hidden ${
                isOpen
                  ? 'border-[#3B4FD9]/40 shadow-soft'
                  : 'border-[#EDEAFB] hover:border-[#3B4FD9]/30'
              }`}
            >
              <button
              ...
              >
                <span className="text-sm sm:text-base font-extrabold text-[#151235]">
                  {faq.question}
                </span>
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-transform duration-200 ${
                    isOpen ? 'bg-[#EDEAFB] text-[#5B4FE0] rotate-180' : 'bg-[#F6F5FC] text-[#5B5876]'
                  }`}
                >
              ...
              <AnimatePresence initial={false}>
                {isOpen && (
                  ...
                  >
                    <div className="px-5 sm:px-6 pb-5 sm:pb-6 text-xs sm:text-sm text-[#5B5876] leading-relaxed border-t border-[#EDEAFB] pt-3">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
