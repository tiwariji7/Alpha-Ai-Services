...
export const PlanComparison: React.FC = () => {
  const renderCellContent = (value: string) => {
    if (value.startsWith('✓')) {
      return (
        <span className="inline-flex items-center gap-1 text-[#111111] font-bold">
    ...
    return <span className="font-medium text-[#111111]">{value}</span>;
  };
  ...
  return (
    <section id="plan-comparison-section" className="px-4 sm:px-6 max-w-6xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-[#EDE9E4] shadow-soft overflow-hidden">
        ...
        <div className="overflow-x-auto relative">
          <table className="w-full text-left border-collapse text-xs sm:text-sm min-w-[620px] sm:min-w-[720px]">
            <thead>
              <tr className="border-b border-[#EDE9E4] bg-[#FAF8F6]">
                <th className="py-4 px-4 sm:px-6 font-extrabold text-[#111111] sticky left-0 bg-[#FAF8F6] z-10 w-1/3 sm:w-2/5 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                  Capability & Support
                </th>
                <th className="py-4 px-3 sm:px-4 font-extrabold text-[#111111] text-center w-1/5">
                  <div>Digital Starter</div>
                  <div className="text-[11px] font-bold text-[#FF5A1F] mt-0.5">₹9,999/mo</div>
                </th>
                <th className="py-4 px-3 sm:px-4 font-extrabold text-[#111111] text-center w-1/5 bg-[#FF5A1F]/5 border-x border-[#EDE9E4]/60">
                  <div>Business Growth</div>
                  <div className="text-[11px] font-bold text-[#FF5A1F] mt-0.5">₹24,999/mo</div>
                </th>
                <th className="py-4 px-3 sm:px-4 font-extrabold text-[#111111] text-center w-1/5">
                  <div>Tech Partner</div>
                  <div className="text-[11px] font-bold text-[#FF5A1F] mt-0.5">₹49,999/mo</div>
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#EDE9E4]">
              {COMPARISON_TABLE_ROWS.map((row, idx) => {
                ...
                return (
                  <tr
                    key={idx}
                    className={`hover:bg-[#FAF8F6]/80 transition-colors ${
                      isHighlight ? 'bg-[#FF5A1F]/[0.03]' : idx % 2 === 1 ? 'bg-[#FAF8F6]/30' : ''
                    }`}
                  >
                    <td
                      className={`py-3.5 px-4 sm:px-6 font-semibold sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] ${
                        isHighlight
                          ? 'bg-amber-50/50 font-bold text-[#111111]'
                          : idx % 2 === 1
                          ? 'bg-[#FDFCFA] text-[#111111]'
                          : 'bg-white text-[#111111]'
                      }`}
                    >
                      {row.feature}
                    </td>
                    <td className="py-3.5 px-3 sm:px-4 text-center text-[#6B6660]">
                      {renderCellContent(row.starter)}
                    </td>
                    <td className="py-3.5 px-3 sm:px-4 text-center bg-[#FF5A1F]/5 border-x border-[#EDE9E4]/60 font-semibold text-[#111111]">
                      {renderCellContent(row.growth)}
                    </td>
                    <td className="py-3.5 px-3 sm:px-4 text-center text-[#111111] font-semibold">
