          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
              activeFilter === f.id
                ...
                : 'bg-white text-[#6B6660] border border-[#EDE9E4] hover:bg-[#FAF8F6]'
            }`}
          >
            {f.label}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5">
        {getFilteredExamples().map((req, idx) => (
          <motion.div
            ...
            className="p-4 rounded-2xl bg-white border border-[#EDE9E4] hover:border-[#FF5A1F] hover:shadow-soft transition-all text-left flex items-start gap-3 group cursor-pointer"
          >
            <div className="w-6 h-6 rounded-lg bg-[#FF5A1F]/10 text-[#FF5A1F] flex items-center justify-center shrink-0 mt-0.5 group-hover:bg-[#FF5A1F] group-hover:text-white transition-colors">
              <MessageSquareCode className="w-3.5 h-3.5" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-bold text-[#111111] group-hover:text-[#FF5A1F] transition-colors leading-snug">
                "{req}"
              </p>
              <span className="text-[10px] text-[#8C867F] font-medium block">
