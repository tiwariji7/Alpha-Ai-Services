  ...
  const points = [
    {
      ...
      icon: <Workflow className="w-5 h-5 text-[#3B4FD9]" />,
    },
    {
      num: '02',
      ...
      icon: <ShieldAlert className="w-5 h-5 text-[#2A3FA8]" />,
    },
    {
      num: '03',
      ...
      icon: <Wrench className="w-5 h-5 text-[#4D6BFF]" />,
    },
    {
      num: '04',
      ...
      icon: <Compass className="w-5 h-5 text-[#5B4FE0]" />,
    },
  ];
  ...
  return (
    <section className="px-4 sm:px-6 max-w-6xl mx-auto">
      <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-10 border border-[#EDEAFB] shadow-soft text-left space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {points.map((pt, idx) => (
            <motion.div
              ...
              className="bg-[#F6F5FC] p-5 rounded-2xl border border-[#EDEAFB] space-y-3 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between">
                <div className="w-9 h-9 rounded-xl bg-[#EDEAFB] border border-[#7B5CE8]/20 flex items-center justify-center shadow-2xs">
                  {pt.icon}
                </div>
                <span className="text-xs font-extrabold text-[#5B5876]">{pt.num}</span>
              </div>

              <div className="space-y-1.5">
                <h3 className="text-xs sm:text-sm font-extrabold text-[#151235] tracking-wide">
                  {pt.title}
                </h3>
        ...
        <div className="p-4 sm:p-4.5 bg-[#EDEAFB]/50 rounded-2xl border border-[#EDEAFB] flex items-start gap-3 text-xs sm:text-[13px] text-[#5B5876] leading-relaxed">
          <AlertCircle className="w-4 h-4 text-[#3B4FD9] shrink-0 mt-0.5" />
          <span>
            <strong className="text-[#151235]">Important Capacity Note:</strong> Dedicated does not mean unlimited work. Tasks are handled within the agreed monthly capacity and prioritized sprint queue.
          </span>
        </div>
      </div>
    </section>
